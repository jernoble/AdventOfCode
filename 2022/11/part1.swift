...
class Monkey {
    ...
    static func construct() -> Monkey? {
        ...
        guard let testValue = Int(testParts[2]) else { return nil }
        let test = { $0 % testValue == 0 }
        ...
        let monkey = Monkey(index: index, startingItems: startingItems, operation: operation, test: test, action: action)
    ...
    init(index: Int, startingItems: [Int], operation: @escaping (_ input: Int) -> Int, test: @escaping (_ input: Int) -> Bool, action: @escaping (_ input: Bool) -> Int) {
        self.index = index
        items = startingItems
