...
struct Map {
    ...
    func adjacentPoints(to: Point) -> Set<Point> {
        ...
        if southPoint.y < grid.count && elevationChange(from: southPoint, to: to) <= 1 { adjacentPoints.insert(southPoint) }

        let northPoint = to + Point(x: 0, y: -1)
        if northPoint.y >= 0 && elevationChange(from: northPoint, to: to) <= 1 { adjacentPoints.insert(northPoint) }

        let eastPoint = to + Point(x: 1, y: 0)
        if eastPoint.x < grid[0].count && elevationChange(from: eastPoint, to: to) <= 1 { adjacentPoints.insert(eastPoint) }

        let westPoint = to + Point(x: -1, y: 0)
        if westPoint.x >= 0 && elevationChange(from: westPoint, to: to) <= 1 { adjacentPoints.insert(westPoint) }
    ...
    func elevations(of: Set<Point>) -> [Character] {
        var results = [Character]()
        for at in of { results.append(grid[at.y][at.x].elevation) }
        return results
    }
}
...
let elapsed = ContinuousClock().measure {
    ...
    while true {
        ...
        if map.elevations(of: newPoints).contains("a") { break }
        round += 1
    }
    let score = round
    print("Score: \(score)")
