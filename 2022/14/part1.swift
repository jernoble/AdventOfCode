...
class Map: CustomStringConvertible {
    ...
    func contains(_ point: Point) -> Bool {
        return point.x >= 0 && point.x < width && point.y >= 0 && point.y < grid.count
    }

    func isEmpty(_ point: Point) -> Bool {
        return contains(point) && !grid[point.y][point.x]
    }

    func drip(at point: Point) -> Bool {
        var point = point
        while true {
            var nextPoint = point + Point(x: 0, y: 1)
            if !contains(nextPoint) { return false }
    ...
    var description: String {
        var output = String()
        for row in grid {
            output.append(String(row.suffix(10).map { $0 ? "#" : "." }))
            output.append("\n")
        }
        return output
    }
