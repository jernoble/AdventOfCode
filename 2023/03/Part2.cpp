...
int main(int argc, char *argv[])
{
    ...
    int64_t score = 0;
    for (auto& gearLocation : gearLocations) {
        auto matchingParts = [&](auto& numberLocation) {
            return gearLocation.y >= numberLocation.second.y - 1
                && gearLocation.y <= numberLocation.second.y + 1
                && gearLocation.x >= numberLocation.second.minX() - 1
                && gearLocation.x <= numberLocation.second.maxX() + 1;
        };

        auto parts = views::filter(numberLocations, matchingParts);
        ...
        score += first.first * second.first;
    }
    printf("Total: %lld\n", score);
